 ...
 *
 * The followings are the available columns in table 'detailpenjualan':
 * @property integer $id
...
class Detailpenjualan extends CActiveRecord
{
	...
	public function summaryToday(){
		$today = Yii::app()->db->createCommand('
			...
			FROM detail_penjualan a LEFT JOIN penjualan b ON a.group_id=b.id 
			WHERE b.tanggal_jual=DATE(NOW())
			GROUP BY b.tanggal_jual
			')->queryScalar();
		return  $today;
	}
	...
	public function summaryWeek(){
		$week = Yii::app()->db->createCommand('
			SELECT SUM(a.jumlah) AS summaryToday
			FROM detail_penjualan a LEFT JOIN penjualan b ON a.group_id=b.id 
			WHERE YEARWEEK(b.tanggal_jual)=YEARWEEK(NOW())
			GROUP BY YEARWEEK(b.tanggal_jual);
			')->queryScalar();
		return  $week;
	}
	...
	public function summaryMonth(){
		$month = Yii::app()->db->createCommand('
			SELECT SUM(a.jumlah) AS summaryToday
			FROM detail_penjualan a LEFT JOIN penjualan b ON a.group_id=b.id 
			WHERE MONTH(b.tanggal_jual)=MONTH(NOW())
			GROUP BY MONTH(b.tanggal_jual)
			')->queryScalar();
		return  $month;
	}
	...
	public function summaryYear(){
		$year = Yii::app()->db->createCommand('
			SELECT SUM(a.jumlah) AS summaryToday
			FROM detail_penjualan a LEFT JOIN penjualan b ON a.group_id=b.id 
			WHERE YEAR(b.tanggal_jual)=YEAR(NOW())
			GROUP BY YEAR(b.tanggal_jual)
			')->queryScalar();
		return $year;
	}	
